...
RESULTS_PATH = "style_transfer_qwen_k.json"  # different default out name
...
def generate_description(prompt: str,
    ...
    for ex_content_path, ex_style_path, ex_result_path in example_triplets:
        ...
        messages.append({
            ...
            "content": [
                {"type": "text", "text": "\nEXAMPLE START\nCONTENT IMAGE:"},
                {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{ex_content_b64}"}},
                {"type": "text", "text": "STYLE IMAGE:"},
                {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{ex_style_b64}"}},
                {"type": "text", "text": "STYLIZED RESULT:\n"},
                {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{ex_result_b64}"}},
                {"type": "text", "text": "EXAMPLE END\n"}
            ]
        })
    ...
    messages.append({
        ...
        "content": [
            ...
            {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{main_content_b64}"}},
            {"type": "text", "text": "NEW PAIR — STYLE IMAGE:"},
            {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{main_style_b64}"}},
            {"type": "text", "text": "Now produce ONLY the description (2–5 sentences)."}
        ]
    })
...
def process_record_wrapper(records: List[Dict[str, Any]], idx: int, k: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "example_image_name": example_refs,              # differs from original field name to reflect no image suffix
