...
# DPR encoders
dpr_question_encoder = SentenceTransformer("facebook/dpr-question_encoder-single-nq-base")
dpr_context_encoder = SentenceTransformer("facebook/dpr-ctx_encoder-single-nq-base")
...
def build_dense_index(dataset):
    """Precompute DPR context embeddings for all captions in the dataset."""
    captions = [str(item.get("caption", "")).strip() for item in dataset]
    captions = [c if c else "[EMPTY]" for c in captions]
    embeddings = dpr_context_encoder.encode(
        captions, convert_to_tensor=True, normalize_embeddings=True, batch_size=8
    )
    return embeddings


def retrieve_top_k_dense(dataset, dataset_embeddings, target_caption, k):
    """Retrieve top-k examples using DPR embeddings and cosine similarity."""
    if not target_caption.strip():
        return []

    target_embedding = dpr_question_encoder.encode(
        target_caption, convert_to_tensor=True, normalize_embeddings=True
    )
...
def process_item(test_item, dataset, dataset_embeddings, k_shots, model_name=MODEL_NAME):
    """Process a single dataset item: retrieve K shots, build permutations and call the model."""
    results = []
    ...
    selected_examples = [
        e for e in selected_examples if os.path.exists(e.get("image_path", ""))
    ]
...
def main():
    parser = argparse.ArgumentParser(description="Run the VLM experiment with DPR dense retriever.")
    ...
    output_path = f"{OUTPUT_DIR}/top_{K_SHOTS}_{artist_name}_gemma_dense_dpr.json"
