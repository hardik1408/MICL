...
def build_bm25(dataset):
    """Build a BM25 retriever from dataset captions."""
    tokenized_captions = [item.get("caption", "").lower().split() for item in dataset]
    bm25 = BM25Okapi(tokenized_captions)
    return bm25, tokenized_captions


def retrieve_top_k_bm25(dataset, bm25, tokenized_captions, target_caption, k):
    """Retrieve top-k examples using BM25 similarity."""
    query_tokens = target_caption.lower().split()
    ...
    for idx in ranked_indices:
        cand = dataset[idx]
        if cand.get("caption", "") == target_caption:
            continue  # skip the same caption
...
def process_item(test_item, dataset, bm25, tokenized_captions, k_shots, model_name=MODEL_NAME):
    results = []
    test_image_path = test_item.get("image_path")
    test_caption = test_item.get("caption", "")

    selected_examples = retrieve_top_k_bm25(dataset, bm25, tokenized_captions, test_caption, k_shots)
...
def main():
    parser = argparse.ArgumentParser(description="Run the VLM experiment with BM25 retriever and multithreading.")
    ...
    bm25, tokenized_captions = build_bm25(dataset)

    results = []

    with ThreadPoolExecutor(max_workers=THREADS) as executor:
        future_to_item = {
            executor.submit(process_item, item, dataset, bm25, tokenized_captions, K_SHOTS): item
            for item in dataset
        }
    ...
    output_path = f"{OUTPUT_DIR}/top_{K_SHOTS}_{artist_name}_gemma_bm25.json"
