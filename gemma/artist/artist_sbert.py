...
def build_dense_index(dataset):
    """Precompute SBERT embeddings for all captions in the dataset."""
    captions = [item.get("caption", "") for item in dataset]
    embeddings = sbert_model.encode(captions, convert_to_tensor=True, normalize_embeddings=True)
    return embeddings


def retrieve_top_k_dense(dataset, dataset_embeddings, target_caption, k):
    """Retrieve top-k examples using SBERT embeddings and cosine similarity."""
    if not target_caption.strip():
        return []

    target_embedding = sbert_model.encode(
...
def main():
    parser = argparse.ArgumentParser(description="Run the VLM experiment with SBERT dense retriever.")
    ...
    output_path = f"{OUTPUT_DIR}/top_{K_SHOTS}_{artist_name}_gemma_dense_sbert.json"
